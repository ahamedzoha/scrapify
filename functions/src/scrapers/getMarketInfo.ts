...
const getMarketInfo: GetMarketInfo = async () => {
  try {
    ...
    $('div.col-md-6.col-xs-12.col-sm-12.LeftColHome > ._row > .midrow').each(
      (i, el) => {
        const indexName = $(el)
          .find('div.m_col-1')
          .text()
        ...
        while (i < 3) {
          // check length of indexName and others and return error if empty
          if (
            indexName.length <= 1 ||
            indexValue.length <= 1 ||
            indexChange.length <= 1 ||
            indexChangePercent.length <= 1
          ) {
            throw new Error('Market Index Data is not available')
          }
